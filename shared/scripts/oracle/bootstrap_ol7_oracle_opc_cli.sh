...
awk -v opc_api=${opc_api_var} -v opc_user=${opc_user_var} -f /tmp/scripts/oracle/enable_ol7_oracle_opc_cli.awk /tmp/scripts/oracle/users/user-vagrant-bashrc.sh > .bashrc.${curdate}.opc-cli
