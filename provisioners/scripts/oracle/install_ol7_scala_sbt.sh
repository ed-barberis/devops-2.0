...
grep -qF "${sbt_env_name}" .bashrc || awk -v env_comment=${sbt_env_comment} -v env_name=${sbt_env_name} -v env_value=${sbt_env_value} -f /tmp/scripts/oracle/append_ol7_env_path.awk .bashrc > .bashrc.${curdate}.${sbthome}
