#!/bin/sh -eux
#---------------------------------------------------------------------------------------------------
# Install MySQL Shell 9.7 by Oracle on CentOS Linux 10.x.
#
...
mysqlsh_release="${mysqlsh_release:-9.7.1-1}"                               # [optional] mysql release version (defaults to '9.7.1-1').
...
if [ "$cpu_arch" = "x86_64" ]; then
  ...
  mysqlsh_checksum="${mysqlsh_checksum:-6ff0dd13403204a9fc6ef6ba632db064}"  # [optional] mysql shell repository amd64 md5 checksum (defaults to published value).
elif [ "$cpu_arch" = "aarch64" ]; then
  # set the arm64 variables.
  mysqlsh_checksum="${mysqlsh_checksum:-b98bfb0ddde425cf559fb8abd0cd085b}"  # [optional] mysql shell repository arm64 md5 checksum (defaults to published value).
else
