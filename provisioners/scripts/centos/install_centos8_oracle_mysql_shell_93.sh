#!/bin/sh -eux
#---------------------------------------------------------------------------------------------------
# Install MySQL Shell 9.3 by Oracle on CentOS Linux 8.x.
#
...
mysqlsh_release="${mysqlsh_release:-9.3.0-1}"                               # [optional] mysql release version (defaults to '9.3.0-1').
...
if [ "$cpu_arch" = "x86_64" ]; then
  ...
  mysqlsh_checksum="${mysqlsh_checksum:-cc41821177a2dca49159cdad7ebb9256}"  # [optional] mysql shell repository amd64 md5 checksum (defaults to published value).
elif [ "$cpu_arch" = "aarch64" ]; then
  # set the arm64 variables.
  mysqlsh_checksum="${mysqlsh_checksum:-c2b9ba3e087c771f095a9fcdf51adc1d}"  # [optional] mysql shell repository arm64 md5 checksum (defaults to published value).
else
...
#   This is the MySQL Shell User Guide extract from the MySQL 9.3 Reference Manual.
#   https://dev.mysql.com/doc/mysql-shell-excerpt/9.3/en/
#
