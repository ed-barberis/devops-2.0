#!/bin/sh -eux
#---------------------------------------------------------------------------------------------------
# Install MySQL Shell 8.4 LTS by Oracle on CentOS Linux 10.x.
#
...
mysqlsh_release="${mysqlsh_release:-8.4.6-1}"                               # [optional] mysql release version (defaults to '8.4.6-1').
...
if [ "$cpu_arch" = "x86_64" ]; then
  ...
  mysqlsh_checksum="${mysqlsh_checksum:-f10cf19ae4e075f04fb1bc68032a6a73}"  # [optional] mysql shell repository amd64 md5 checksum (defaults to published value).
elif [ "$cpu_arch" = "aarch64" ]; then
  # set the arm64 variables.
  mysqlsh_checksum="${mysqlsh_checksum:-ef7adbf6e61d7b86768f363562be8e07}"  # [optional] mysql shell repository arm64 md5 checksum (defaults to published value).
else
