#!/bin/sh -eux
#---------------------------------------------------------------------------------------------------
# Install MySQL Shell 9.4 by Oracle on CentOS Linux 10.x.
#
...
mysqlsh_release="${mysqlsh_release:-9.4.0-1}"                               # [optional] mysql release version (defaults to '9.4.0-1').
...
if [ "$cpu_arch" = "x86_64" ]; then
  ...
  mysqlsh_checksum="${mysqlsh_checksum:-f618bdec1c49a4eedea294d70af4f4e8}"  # [optional] mysql shell repository amd64 md5 checksum (defaults to published value).
elif [ "$cpu_arch" = "aarch64" ]; then
  # set the arm64 variables.
  mysqlsh_checksum="${mysqlsh_checksum:-ff66397e3672fec6524462db5dcaddda}"  # [optional] mysql shell repository arm64 md5 checksum (defaults to published value).
else
...
# install mysql shell. -----------------------------------------------------------------------------
mysqlsh_binary="mysql-shell-${mysqlsh_release}.el10.${cpu_arch}.rpm"
...
# mysql-shell-${mysqlsh_release}.el10.x86_64.rpm: OK
# mysql-shell-${mysqlsh_release}.el10.aarch64.rpm: OK
...
#   This is the MySQL Shell User Guide extract from the MySQL 9.4 Reference Manual.
#   https://dev.mysql.com/doc/mysql-shell-excerpt/9.4/en/
#
