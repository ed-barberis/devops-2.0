...
curl --silent --dump-header curl-postman-linux-x64.${curdate}.out https://dl.pstmn.io/download/latest/linux?arch=64 --output ${postmanbinary}
postmanrelease=$(awk '{ sub("\r$", ""); print }' curl-postman-linux-x64.${curdate}.out | awk -F "=" '/filename/ {print $2}')
mv ${postmanbinary} ${postmanrelease}
rm -f curl-postman-linux-x64.${curdate}.out
