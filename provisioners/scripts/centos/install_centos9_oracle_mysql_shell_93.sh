#!/bin/sh -eux
#---------------------------------------------------------------------------------------------------
# Install MySQL Shell 9.2 by Oracle on CentOS Linux 9.x.
#
...
if [ "$cpu_arch" = "x86_64" ]; then
  ...
  mysqlsh_checksum="${mysqlsh_checksum:-1d22f5b134cd1f344adc568ae87330b5}"  # [optional] mysql shell repository amd64 md5 checksum (defaults to published value).
elif [ "$cpu_arch" = "aarch64" ]; then
  # set the arm64 variables.
  mysqlsh_checksum="${mysqlsh_checksum:-9fdc8ac7be93ddff22df3212700472b9}"  # [optional] mysql shell repository arm64 md5 checksum (defaults to published value).
else
