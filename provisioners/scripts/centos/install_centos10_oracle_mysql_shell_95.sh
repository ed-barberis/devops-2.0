#!/bin/sh -eux
#---------------------------------------------------------------------------------------------------
# Install MySQL Shell 9.6 by Oracle on CentOS Linux 10.x.
#
...
mysqlsh_release="${mysqlsh_release:-9.6.0-1}"                               # [optional] mysql release version (defaults to '9.6.0-1').
...
if [ "$cpu_arch" = "x86_64" ]; then
  ...
  mysqlsh_checksum="${mysqlsh_checksum:-b3112470b5ad6e3a90be67eb6488936f}"  # [optional] mysql shell repository amd64 md5 checksum (defaults to published value).
elif [ "$cpu_arch" = "aarch64" ]; then
  # set the arm64 variables.
  mysqlsh_checksum="${mysqlsh_checksum:-46e08691919ed43a4e9e03d8cea9c321}"  # [optional] mysql shell repository arm64 md5 checksum (defaults to published value).
else
