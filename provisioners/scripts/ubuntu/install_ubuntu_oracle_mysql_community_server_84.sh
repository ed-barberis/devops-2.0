#!/bin/bash -eux
#---------------------------------------------------------------------------------------------------
# Install MySQL Community Server 8.4 LTS by Oracle on Ubuntu Linux.
#
...
if [ -n "$ubuntu_release" ]; then
  case $ubuntu_release in
      ...
      *)
        echo "Error: MySQL Community Server 8.4 LTS NOT supported on Ubuntu release: '$(lsb_release -ds)'."
        exit 1
        ;;
  esac
fi
