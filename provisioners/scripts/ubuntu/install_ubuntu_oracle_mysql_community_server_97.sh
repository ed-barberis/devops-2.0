#!/bin/bash -eux
#---------------------------------------------------------------------------------------------------
# Install MySQL Community Server 9.7 by Oracle on Ubuntu Linux.
#
...
mysql_server_release="${mysql_server_release:-mysql-9.7-lts}"           # [optional] mysql server version (defaults to 'mysql-9.7-lts').
...
if [ -n "$ubuntu_release" ]; then
  case $ubuntu_release in
      ...
      *)
        echo "Error: MySQL Community Server 9.7 NOT supported on Ubuntu release: '$(lsb_release -ds)'."
        exit 1
        ;;
  esac
fi
